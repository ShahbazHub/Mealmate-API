...
export const adapter: EntityAdapter<CustomerModel> = createEntityAdapter<
  CustomerModel
>();
...
export function customersReducer(
  ...
  switch (action.type) {
    ...
    case CustomerActionTypes.CustomerCreated:
      return adapter.addOne(action.payload.customer, {
        ...state,
        lastCreatedCustomerId: action.payload.customer.id,
      });
...
export const getCustomerState = createFeatureSelector<CustomerModel>(
  "customers"
);
