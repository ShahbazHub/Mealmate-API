...
export class CustomerEffects {
  ...
  showActionLoadingDistpatcher = new CustomerActionToggleLoading({
    isLoading: true,
  });
  hideActionLoadingDistpatcher = new CustomerActionToggleLoading({
    isLoading: false,
  });
  ...
  createCustomer$ = this.actions$.pipe(
    ofType<CustomerOnServerCreated>(
      CustomerActionTypes.CustomerOnServerCreated
    ),
    mergeMap(({ payload }) => {
      this.store.dispatch(this.showActionLoadingDistpatcher);
